...
def FirnAir_TR(z_edges_vec,z_P_vec,nt,dt,Gamma_P,bc_u,bc_d,phi_0,rhoHL, R,nz_P,nz_fv,por_op,gas,Accu_0,T,p_a,por_tot,por_cl,Accu_m):
    ...
    f_depth=np.loadtxt(os.path.join(DataPath,'depthDART.csv'),delimiter=',',skiprows=0)
    #f_depth=f_depth[:,1:]
    #f_depth=f_depth[0:1996,1:] #what is 1996?

    f_density=np.loadtxt(os.path.join(DataPath,'densityDART.csv'),delimiter=',',skiprows=0)
