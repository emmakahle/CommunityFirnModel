...
spot = os.path.dirname(sys.argv[0]) # directory name of the script
print spot 
# os.chdir(spot) # change pwd to location of firnmodel.py
sys.path.insert(1, spot) # add the script directory to the search path for modules
ResultsPlace = os.path.join(spot, 'Results') # concatenate the string 'Results' to the script directory name 
DataPath = os.path.join(spot, 'DataImport') # concatenate the string 'DataImport' to the script directory name 
sys.path.insert(3, DataImport) # add the DataImport folder to the search path for modules 
os.chdir(/Users/huongnvo/Dropbox/Glaciology/CommunityFirnModel/code/firnmodel) # my code for changing directory 

np.set_printoptions(linewidth = 300) # set output reading to be wider. A bit easier to read :)

# for using config.json 
with open('config.json') as f:
   c = json.load(f)

# Downward Advection (of air)        
def w(Accu, rho_interface, por_op, T, p_a, por_tot, por_cl): # Function for downward advection of air. 
    ...
    w_ice = Accu * c['rho_i'] / rho_interface
    
    if c['ad_method'] == 'ice_vel':
        w_ad = w_ice
    
    elif c['ad_method'] == 'Christo':
    ### Christo's Method from his thesis (chapter 5). This (maybe) could be vectorized to speed it up.
        ...
        C = np.exp(c['M_air'] * g * z_edges_vec / (c['R'] * T))
...
def FirnAir_SS(nt, dt, Gamma_P, bc_u, bc_d, phi_0, rhoHL, nz_P, nz_fv, por_op, gas, Accu, T, p_a, diffu, d_eddy):
    ...
    S_C_0 = (-diffu_d + diffu_u) * (deltaM * g / (c['R'] * T)) #S_C is independent source term in Patankar
    ...
    S_P = (-diffu_d + diffu_u) * (deltaM * g / (c['R'] * T)) #gravity term, S_P is phi-dependent source
    ...
    w_edges = w(z_edges_vec, z_P_vec, Accu, rho_interface, por_op, T, p_a, por_tot, por_cl)
...
def FirnAir_TR(z_edges_vec, z_P_vec, nt, dt, Gamma_P, bc_u, bc_d, phi_0, rhoHL, nz_P, nz_fv, por_op, gas, Accu_0, T, p_a, por_tot, por_cl, Accu_m, czd):
    ...
    #f_depth = np.loadtxt(os.path.join(DataPath, 'depthDART2.csv'), delimiter = ',', skiprows = 0)
    
    f_depth = np.loadtxt('depthDART2.csv', delimiter = ',', skiprows = 0)
    f_density = np.loadtxt('densityDART2.csv', delimiter = ',', skiprows = 0)
    f_dcon = np.loadtxt('DconDART2.csv', delimiter = ',', skiprows = 0)

    
    #load data from firnmodel.py. This is what should be streamlined.
    f_depth=f_depth[:,1:]
...
def porosity(rho_prof):
    ...
    bcoRho = 1 / (1 / (c['rho_i']) + T * 6.95e-7 - 4.3e-5) # Martinerie density at close off; see Buizert thesis (2011), Blunier & Schwander (2000), Goujon (2003)
    LIDRho = bcoRho - 14.0 #LIZ depth (Blunier and Schwander, 2000)
    
    ## Porosity, from Goujon et al., 2003, equations 9 and 10
    por_tot = 1 - rho_prof / c['rho_i'] # Total porosity
...
#def rhoHLAnalytic(R,T,rho0,z_nodes,Accu_m): 
#    
#    #if Accu is None:
#    Accu=Accu_m   
#    rho_c = 550.0/1000.
...
if __name__ == "__main__":
    ...
    #sitechoice = 'SCENARIO'
    #sitechoice = 'NEEM'
    g, p_a, T, Accu_0, czd, z_co, LIZ, rho0, hemisphere = MPS.sites(c['sitechoice'])   
    
    Accu_m = Accu_0 #Accumulation in m/year
    
    Accu_0 = Accu_0 / c['sPerYear'] #accumulation in m/second
    ...
    #gaschoice = 'SF6'    
    #gaschoice = 'd15N2'
    D_x, M, deltaM, conc1, firn_meas, d_0 = MPG.gasses(c['gaschoice'], c['sitechoice'], T, p_a, DataPath, hemisphere, measurements)
    ...
    rhoHL = MPRHO.rhoHLAnalytic(T, rho0, rho_bco, z_nodes, Accu_m) # Get density profile from H&L analytic
    rho_co, por_co, por_tot, por_cl, por_op, bcoRho, LIDRho = porosity(rhoHL)
    
    if c['sitechoice'] == 'SCENARIO':
    ...
    if transdiffu == 'on':
        ...
        phi, a_P_out = FirnAir_SS(z_edges_vec, z_P_vec, nt, dt, Gamma_P, bc_u, bc_d, phi_0, rhoHL, nz_P, nz_fv, por_op, gas, Accu_0, T, p_a, diffu, d_eddy)
